...
export default {
  Player: class extends PlayerModel {
    constructor(game) {
      ...
      ((this.base = new SpriteNode(
        this.game,
        "./static/images/Entity/Player/Player.svg",
      )),
    ...
    update(t, e) {
      ...
      if (e) {
        ...
        if (e.zombieShieldMaxHealth > 0) {
          this.shieldBar.setVisible(
            e.zombieShieldHealth < e.zombieShieldMaxHealth,
          );
        } else {
          this.shieldBar.setVisible(false);
        }
    ...
    updateBowWeapon(t = 500, e = 250) {
      return (r, n) => {
        ...
        if ((this.setRotation(i), r.startChargingTick)) {
          ...
          let e = this.game.renderer.replicator.getMsSinceTick(
              r.startChargingTick,
            ),
            n = Math.min(e / t, 1);
        ...
        case "Sword":
          ...
          (e.setAnchor(0.5, 0.9),
            ...
            (this.weaponUpdateFunc = this.updateAntiClockwiseSwingingWeapon(
              this.game.ui.toolData[t.weaponName].msBetweenFires[
                t.weaponTier - 1
              ],
              100,
            )));
    ...
    updateModel(t = 1) {
      if (t != this.currentTier) {
        if (
          ...
          throw new Error(
            `Unknown tier encountered for ${this.type} tower: ${t}`,
          );
        ((this.base = new SpriteNode(
          this.game,
  ...
  RocketProjectile: class extends Model {
    ...
    onDie() {
      ...
      let r = 0;
      const n =
        this.game.ui.buildingData.RocketTower.projectileAoeRadius[
          this.tier - 1
        ];
      t.update = (i) => {
        (t.draw.clear(),
