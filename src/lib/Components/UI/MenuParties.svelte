        ...
        {#each tabs as tab}
            <button
                class="{currentTab == tab
                    ? 'active'
                    : ''} bg-black/40 text-white/70 text-xs text-center p-2 pl-3 pr-3 transition first:rounded-tl-md last:rounded-tr-md hover:bg-black/20 cursor-pointer"
                onclick={() => {
                    currentTab = tab;
                    if (tab === "Your Instances") {
                        invoke("open_instances_window");
                    }
                }}>{tab}</button
            >
        {/each}
