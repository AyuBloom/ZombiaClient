class ni {
  constructor() {
    ((this.socket = null),
      ...
      (this.languageFilterEnabled = !0),
      (this.currentTickNumber = 0),
      (this.knownEntities = []),
      (this.packetArr = []),
      (this.packetCountLimit = 200),
      (this.inputPacketManager = new Qe()),
      (this.modelProps = {
        Projectile: {
          name: "Projectile",
          index: 0,
  ...
  encode(t, e) {
    const i = new (ei())(100, !0);
    switch ((i.writeUint8(t), t)) {
      case 4:
        this.encodeEnterWorld(i, e);
        break;
      case 3:
        this.encodeInput(i, e);
        break;
      case 9:
        this.encodeRpc(i, e);
    }
    return (i.flip(), i.compact(), i.toArrayBuffer(!1));
  }
  ...
  decode(t) {
    ...
    e.littleEndian = !0;
    const i = e.readUint8();
    if (0 == i && "hidden" == document.visibilityState)
      return (
    ...
    switch (i) {
      case 4:
        ...
        break;
      case 0:
        if (((s = this.decodeEntityUpdate(e)), 1 == s.unsynced)) return {};
        break;
      case 9:
        s = this.decodeRpc(e);
        break;
  ...
  decodeEntityUpdate(t) {
    ...
    else if (!1 === i && !0 === this.outOfSync)
      return {
        unsynced: !0,
      };
    let s = {};
    for (let t of this.knownEntities) s[t] = !0;
    const n = t.readVarint32();
    for (let e = 0; e < n; e++) delete s[t.readUint32()];
    ...
    for (let e = 0; e < r; e++) {
      ...
      const n =
        e === mi.renderer.world.localPlayer
          ? i.privateProps
    ...
    return (
      ...
      !0 === mi.ui.components.uiDevHud?.devEnabled &&
        (o = t.readUint16() / 100),
      {
        tick: e,
        entities: s,
  ...
  onServerDesync() {
    ((this.packetArr.length = 0),
      (this.outOfSync = !0),
      (this.knownEntities = []),
      mi.renderer.onServerDesync(),
      mi.renderer.world.onServerDesync(),
      mi.renderer.replicator.onServerDesync());
  }
  init() {
      ...
      (document.onvisibilitychange = () => {
        if (
          ...
          if (this.packetArr.length >= this.packetCountLimit)
            return (
              console.log(
                "Tab was hidden for too long. Reporting as desynced.",
              ),
              (this.packetArr.length = 0),
              this.onServerDesync(),
              void mi.network.sendRpc({
                name: "OutOfSync",
              })
            );
          for (
  ...
  connect() {
    window.self === window.top &&
      ...
      (this.connected ||
        this.connecting ||
        ((this.connecting = !0),
        "Local" == this.options.serverData.country
  ...
  onSocketOpen() {
    ((this.connected = !0),
      ...
      this.sendEnterWorld(
        this.options.name,
        this.options.partyKey,
        this.reconnectSecret,
      ),
      this.sendPingIfNecessary(),
      mi.eventEmitter.emit("SocketOpened"));
  }
  onSocketClose() {
    (this.onServerDesync(),
      (this.packetArr.length = 0),
      ...
      this.socketIntentionallyClosed
        ? (this.socketIntentionallyClosed = !1)
        : setTimeout(() => {
            this.connect();
          }, 1e3),
      mi.eventEmitter.emit("SocketClosed"));
  }
  onSocketMessage(t) {
  ...
  sendPacket(t, e) {
    this.connected && this.socket.send(this.encode(t, e));
  }
  sendEnterWorld(t, e, i) {
    this.sendPacket(4, {
      name: t,
      partyKey: e,
      reconnectSecret: i,
  ...
  handleEnterWorldResponse(t) {
    if (!1 === t.allowed)
      return (
        mi.ui.components.uiIntro.setFailure(t),
        void (this.socketIntentionallyClosed = !0)
      );
    ((this.currentTickNumber = t.startingTick),
      mi.eventEmitter.emit("EnterWorldResponse", t));
  }
  handleEntityUpdate(t) {
    const e = performance.now();
    ((this.entityUpdateTimeDelta = e - this.lastEntityUpdateMessageReceived),
      (this.lastEntityUpdateMessageReceived = e),
      this.sendPingIfNecessary(),
      mi.eventEmitter.emit("EntityUpdate", t));
  }
  handleRpc(t) {
    mi.eventEmitter.emit(`${t.name}RpcReceived`, t.response);
  }
  handlePing() {
    const t = new Date();
