...
class Server {
        ...
        try {
          ...
          socket.on("close", () => {
            ...
            if (socket.uid !== undefined) {
              ...
              this.parties[playerEntity.partyId].removeMember(
                this,
                playerEntity,
              );
            }
      ...
      */
      ...
      this.physicsHooks = {
        filterContactPair(colliderHandle1, colliderHandle2) {
          ...
          if (
            entity1.model == "Player" &&
            entity1.canWalkThroughBuildings == true
          ) {
            if (
      ...
      for (const model in this.pooledEntities) {
        ...
        this.pooledEntities[model].entities.grow(
          this.pooledEntities[model].countToPool,
        );
      }

      const generateWeightedPosition = () => {
        const x = Math.round(
          Math.random() * this.serverProperties.mapSize.width,
        );
        const y = Math.round(
          Math.random() * this.serverProperties.mapSize.height,
        );
  ...
  createEntity(model, props = {}, entityToBePooled = false) {
    if (Entities[model] === undefined)
      return this.debugLog(
        `[ERROR] Type ${model} cannot be found in known entities.`,
      );
    ...
    if (props.position !== undefined) {
      props.position = {
        x: Math.round(
          Math.max(
            0,
            Math.min(this.serverProperties.mapSize.width, props.position.x),
          ),
        ),
        y: Math.round(
          Math.max(
            0,
            Math.min(this.serverProperties.mapSize.height, props.position.y),
          ),
        ),
      };
    }
    ...
    if (entityToBePooled === true) {
      ...
      this.colliderHandleEntityMap[entity.physicsObject.collider(0).handle] =
        entity;
  ...
  removeEntity(uid) {
    ...
    delete this.colliderHandleEntityMap[
      entity.physicsObject.collider(0).handle
    ];
  ...
  sendPartyData() {
    ...
    this.waitTicks(
      1000 / this.serverProperties.tickRate,
      this.sendPartyData.bind(this),
    );
  }
  ...
  getPartyData(uid) {
    ...
    for (let id in this.parties) {
      ...
      if (
        party.id === this.entities[uid].partyId ||
        party.isOpen === true ||
        isDev
      ) {
        parties.push({
  ...
  onTick() {
    ...
    for (let uid in this.connectedSockets) {
      ...
      if (this.cachedEntityUpdateData[uid])
        socket.send(this.cachedEntityUpdateData[uid]);
    }
