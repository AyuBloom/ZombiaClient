...
class Grid {
  constructor(server, width, height, offset) {
    ...
    for (
      let x = offset.x - this.width / 2;
      x < offset.x + this.width / 2;
      x++
    ) {
      if (x < 0 || x >= this.mapSize.width) continue;
      this.nodes[x] = {};
      for (
        let y = offset.y - this.height / 2;
        y < offset.y + this.height / 2;
        y++
      ) {
