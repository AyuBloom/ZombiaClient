...
module.exports = UpdateHarvesterTarget = (socket, server, data) => {
  ...
  // Check range
  const harvesterBuildingInfo = BuildingInfo.getBuilding(
    server,
    "Harvester",
  ...
  const distToTarget =
    Math.sqrt(
      Util.measureDistance(
        server,
        harvesterEntity.getPosition(server),
        targetResource.getPosition(server),
      ),
    ) * server.world.PixelToWorld;

  if (distToTarget + targetResource.radius > harvesterBuildingInfo.harvestRange)
    return;
