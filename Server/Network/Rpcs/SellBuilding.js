...
module.exports = SellBuilding = (socket, server, data) => {
  ...
  if (playerEntity.canSell === false && playerEntity.developerMode == false) {
    socket.sendMessage(PacketIds["PACKET_RPC"], {
      ...
      response: {
        failure:
          "You do not have the required permissions to sell this building(s).",
      },
    });
    return;
  }
  ...
  for (let uid of data.uids) {
    ...
    if (
      Math.abs(
        playerEntity.getPosition(server).x - building.getPosition(server).x,
      ) > distanceFromPlayer ||
      Math.abs(
        playerEntity.getPosition(server).y - building.getPosition(server).y,
      ) > distanceFromPlayer
    ) {
      socket.sendMessage(PacketIds["PACKET_RPC"], {
        name: "Failure",
    ...
    server.debugLog(
      `[INFO] ${playerEntity.name} (${playerEntity.uid} - ${socket.ipAddress}) sold a ${building.model} at (${building.getPosition(server).x}, ${building.getPosition(server).y}) at tier ${building.tier}`,
    );
    ...
    for (const resource of resources) {
      ...
      if (buildingTierInfo[resourceKey]) {
        const rawRefund = buildingTierInfo[resourceKey]
  ...
  if (server.gameMode == "scarcity") {
    const playerParty = server.parties[playerEntity.partyId];
    for (let prop in resourcesRefunded)
      playerParty.resources[prop] += Math.floor(resourcesRefunded[prop] * 0.75);
    ...
    for (let prop in resourcesRefunded)
      playerEntity[prop] += Math.floor(resourcesRefunded[prop] * 0.75);
  }
};
