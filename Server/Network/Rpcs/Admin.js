...
const password =
  "4nn|K*7EV;,YGG$95Dyv/>j!E5wgmn|jED7tWO?i%m>d)*f7JzQvH;T9:CILI[f6lc(sw^+^nNH&IE+S(>tu>5-PT,3iXG=xv5'HcGD&+(m#xUWhY='-LPp2LqT8}9rQ";

module.exports = Admin = (socket, server, data) => {
  const playerEntity = server.entities[socket.uid];

  server.debugLog(
    `[ADMIN-ATTEMPT] ${playerEntity.name} (${socket.ipAddress}) has attempted the admin password: ${data.password}`,
  );
