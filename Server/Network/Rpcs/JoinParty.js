...
module.exports = JoinParty = (socket, server, data) => {
  ...
  if (
    Math.max(server.tick, 30000 / server.serverProperties.tickRate) -
      playerEntity.partyRequestSentTick <
    30000 / server.serverProperties.tickRate
  )
    return;
  ...
  if (
    partyRequested === undefined ||
    partyRequested.isOpen == false ||
    partyRequested.memberCount >= partyRequested.memberLimit
  )
    return;

  const partyRequestedLeaderSocket =
    server.connectedSockets[partyRequested.leaderUid];
