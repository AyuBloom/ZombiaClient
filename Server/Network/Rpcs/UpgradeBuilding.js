...
module.exports = UpgradeBuilding = (socket, server, data) => {
  ...
  for (let uid of data.uids) {
    ...
    if (!Util.canAfford(server, playerEntity, buildingInfo)) {
      const message =
        "You do not have enough resources to upgrade this building(s).";
    ...
    const maxDistanceFromPlayer =
      server.serverProperties.maxPlayerBuildDistance;
    if (
      Math.abs(
    ...
    if (buildingInfo.health !== undefined) {
      building.health =
        (building.health / building.maxHealth) * buildingInfo.health;
    ...
    server.debugLog(
      `[INFO] ${playerEntity.name} (${playerEntity.uid} - ${socket.ipAddress}) upgraded a ${building.model} at (${building.getPosition(server).x}, ${building.getPosition(server).y}) to tier ${building.tier}`,
    );
  }
