  ...
  if (
    Math.abs(
      playerEntity.getPosition(server).x -
        server.entities[playerParty.primaryBuilding].getPosition(server).x,
    ) > distanceFromPlayer ||
    Math.abs(
      playerEntity.getPosition(server).y -
        server.entities[playerParty.primaryBuilding].getPosition(server).y,
    ) > distanceFromPlayer
  ) {
    return;
  }

  server.entities[playerParty.primaryBuilding].aggroEnabled =
    !server.entities[playerParty.primaryBuilding].aggroEnabled;
};
