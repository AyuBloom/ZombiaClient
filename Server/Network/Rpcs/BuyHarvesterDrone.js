...
module.exports = BuyHarvesterDrone = (socket, server, data) => {
  ...
  if (
    Math.abs(
      playerEntity.getPosition(server).x -
        harvesterEntity.getPosition(server).x,
    ) > maxDistanceFromPlayer ||
    Math.abs(
      playerEntity.getPosition(server).y -
        harvesterEntity.getPosition(server).y,
    ) > maxDistanceFromPlayer
  ) {
    return;
  }
  ...
  if (
    !Util.canAfford(server, playerEntity, {
      goldCosts: buildingInfo.droneGoldCosts,
    })
  ) {
    socket.sendMessage(PacketIds["PACKET_RPC"], {
      name: "Failure",
  ...
  if (
    (harvesterEntity.droneCount >= buildingInfo.maxDrones) &
    !playerEntity.developerMode
  )
    return;

  if (playerEntity.developerMode == false) {
    Util.deductResourceCosts(server, playerEntity, {
      goldCosts: buildingInfo.droneGoldCosts,
    });
  }
