...
const checkPrimaryExists = (server, socket, playerParty) => {
  ...
  if (
    server.entities[playerParty.primaryBuilding] == undefined ||
    server.entities[playerParty.primaryBuilding].dead == true
  ) {
...
module.exports = CastSpell = (socket, server, data) => {
    ...
    case "Timeout":
      {
        ...
        if (server.cycleData.isDay == true) {
          ...
          iconCooldownMs =
            server.cycleData.dayLength +
            ticksToCycleEnd * server.serverProperties.tickRate;
        }
    ...
    case "Rapidfire":
      {
        ...
        const maxDistanceFromPlayer =
          server.serverProperties.maxPlayerSpellCastDistance -
          spellData.radius / server.world.PixelToWorld;

        if (
          Math.abs(
            playerEntity.getPosition(server).x -
              data.x / server.world.PixelToWorld,
          ) > maxDistanceFromPlayer ||
          Math.abs(
            playerEntity.getPosition(server).y -
        ...
        server.waitTicks(
          spellData.buffDurationMs / server.serverProperties.tickRate,
          () => {
  ...
  playerParty.spells[data.spellName].cooldownExpiringTick =
    server.tick + cooldownMs / server.serverProperties.tickRate;
  playerParty.spells[data.spellName].cooldownIconExpiringTick =
    server.tick + iconCooldownMs / server.serverProperties.tickRate;
