...
module.exports = SendChatMessage = (socket, server, data) => {
    ...
    case "Party":
      {
        for (const member of Object.values(playerParty.members)) {
          server.connectedSockets[member.uid].sendMessage(
            PacketIds["PACKET_RPC"],
            {
              name: "ReceiveChatMessage",
              response: {
  ...
  server.debugLog(
    `[INFO] ${playerEntity.name} (${playerEntity.uid} - ${socket.ipAddress}) sent a message: ${message}`,
  );
};
