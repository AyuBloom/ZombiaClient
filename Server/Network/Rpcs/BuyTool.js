...
module.exports = BuyTool = (socket, server, data) => {
  ...
  const playerTool = playerEntity.tools.find(
    (e) => e.toolName == data.toolName,
  );
  ...
  if (playerTool == undefined) {
    ...
    if (toolInfo.name == "HealthPotion")
      return handleHealthPotion(server, socket, playerEntity, toolInfo);

    // Upgrading the armour must change the shield health data
    if (toolInfo.class == "Armour") {
      switch (toolInfo.name) {
        case "ZombieShield":
          playerEntity.zombieShieldMaxHealth = playerEntity.zombieShieldHealth =
            toolInfo.health;
          break;
      }
    }
  ...
  // If the player already owns the tool
    ...
    if (playerEntity.weaponName === data.toolName)
    ...
    if (toolInfo.class == "Armour") {
      switch (toolInfo.name) {
        case "ZombieShield":
          playerEntity.zombieShieldHealth =
            (playerEntity.zombieShieldHealth /
              playerEntity.zombieShieldMaxHealth) *
            toolInfo.health;
