...
module.exports = CancelPartyRequest = (socket, server, data) => {
  ...
  if (
    server.tick - playerEntity.partyRequestSentTick <
    2000 / server.serverProperties.tickRate
  )
    return;
  ...
  if (server.parties[playerEntity.partyRequestId] !== undefined) {
    const partyLeader =
      server.entities[server.parties[playerEntity.partyRequestId].leaderUid];
    if (partyLeader !== undefined) {
      server.connectedSockets[partyLeader.uid].sendMessage(
