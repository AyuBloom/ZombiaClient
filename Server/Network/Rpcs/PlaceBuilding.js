...
module.exports = PlaceBuilding = (socket, server, data) => {
  ...
  if (gridWidth == 1 && gridHeight == 1) {
  ...
  } else if (gridWidth == 2 && gridHeight == 3) {
    ...
    // y % 24 = 0
    // y % 48 = 24
  } else if (gridWidth == 3 && gridHeight == 2) {
    if (data.x % 24 !== 0) return;
  ...
  if (
    data.x < 0 ||
    data.x > server.serverProperties.mapSize.width ||
    data.y < 0 ||
    data.y > server.serverProperties.mapSize.height
  )
    return;
  ...
  if (playerEntity.canPlace === false && playerEntity.developerMode == false) {
    socket.sendMessage(PacketIds["PACKET_RPC"], {
      ...
      response: {
        failure:
          "You do not have the required permissions to place this building.",
      },
    });
    return;
  }
  ...
  if (
    playerParty.allBuildings.types[data.type].owned.length >=
      playerParty.allBuildings.types[data.type].limit &&
  ...
  if (
    playerEntity.developerMode == false &&
    (Math.abs(playerEntity.getPosition(server).x - data.x) >
      distanceFromPlayer ||
      Math.abs(playerEntity.getPosition(server).y - data.y) >
        distanceFromPlayer)
  ) {
    socket.sendMessage(PacketIds["PACKET_RPC"], {
      name: "Failure",
  ...
  if (
    data.x - width / server.world.PixelToWorld / 2 < distanceFromWall ||
    data.y - height / server.world.PixelToWorld / 2 < distanceFromWall ||
    data.x + width / server.world.PixelToWorld / 2 >
      server.serverProperties.mapSize.width / server.world.PixelToWorld -
        distanceFromWall ||
    data.y + height / server.world.PixelToWorld / 2 >
      server.serverProperties.mapSize.height / server.world.PixelToWorld -
        distanceFromWall
  ) {
    socket.sendMessage(PacketIds["PACKET_RPC"], {
      name: "Failure",
  ...
  if (!["Factory", "Harvester"].includes(data.type)) {
    ...
    if (
      Math.abs(
        data.x +
          width / server.world.PixelToWorld / 2 -
          partyPrimary.getPosition(server).x,
      ) > distanceFromPrimary ||
      Math.abs(
        data.x -
          width / server.world.PixelToWorld / 2 -
      ...
      (Math.abs(
        data.y +
          height / server.world.PixelToWorld / 2 -
          partyPrimary.getPosition(server).y,
      ) >
        distanceFromPrimary) |
        (Math.abs(
          data.y -
  ...
  let tooClose = false;
  if (data.type === "Factory") {
    const minFactoryToFactoryDistance =
      server.serverProperties.minFactoryToFactoryDistance;
    const shape = new RAPIER.Cuboid(
      minFactoryToFactoryDistance,
      minFactoryToFactoryDistance,
    );
  ...
  server.debugLog(
    `[INFO] ${playerEntity.name} (${playerEntity.uid} - ${socket.ipAddress}) placed a ${building.model} at (${building.getPosition(server).x}, ${building.getPosition(server).y})`,
  );
};
