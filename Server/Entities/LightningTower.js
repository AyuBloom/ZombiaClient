...
class LightningTower extends Building {
  ...
  fireRanged(server, buildingInfo) {
    ...
    const findTarget = (lastTargetUid) => {
      ...
      for (const cell of this.sortedCellDistances) {
        ...
        for (const uid of cell) {
          ...
          if (
            lastTargetUid !== this.uid &&
            distanceToEnemy >
              (buildingInfo.maximumBeamBridgingDistance /
      ...
      if (closestEnemyUid !== 0) {
        ...
        if (enemyUids.length < buildingInfo.attackTargetLimit)
          findTarget(closestEnemyUid);
      }
    };
    ...
    enemyUids.forEach((targetUid) => {
      ...
      this.targetBeams.push(
        target.getPosition(server).x * server.world.PixelToWorld,
        target.getPosition(server).y * server.world.PixelToWorld,
      );
