...
class ResourcePickup extends Entity {
  ...
  update(server) {
    ...
    if (server.tick - this.tickMade < 1000 / server.serverProperties.tickRate)
      return;
    ...
    if (this.playersInRange.length == 1) {
      ...
      for (const uid of this.playersInRange) {
        ...
        const distToPlayer = Util.measureDistance(
          server,
          this.getPosition(server),
          entity.getPosition(server),
        );
        ...
        if (
          distToPlayer < closestPlayerDist &&
          distToPlayer < this.pickupRange ** 2
        ) {
    ...
    if (this.targetPlayerUid !== 0) {
      ...
      const distToPlayer = Math.sqrt(
        Util.measureDistance(
          server,
          this.getPosition(server),
          targetPlayer.getPosition(server),
        ),
      );
      ...
      if (distToPlayer < 0.5) {
        server.waitTicks(1, () => {
          if (
            ...
            targetPlayer?.entityClass !== "Player"
          )
            return;
      ...
      this.yaw = Math.floor(
        Util.angleTo(
          this.getPosition(server),
          server.entities[this.targetPlayerUid].getPosition(server),
        ),
      );
