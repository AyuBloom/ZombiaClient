...
class Player extends Entity {
  ...
  addToWorld(server) {
    ...
    this.sightRange = new RAPIER.Cuboid(
      EntityInfo.getEntity("Player").sightRange.width /
        server.world.PixelToWorld,
      EntityInfo.getEntity("Player").sightRange.height /
        server.world.PixelToWorld,
    );

    this.physicsObject.setTranslation(
      {
        x: this.position.x / server.world.PixelToWorld,
        y: this.position.y / server.world.PixelToWorld,
      },
      true,
    );

    const collider = RAPIER.ColliderDesc.ball(
      this.radius / server.world.PixelToWorld,
  ...
  respawn(server) {
    ...
    if (playerParty.primaryBuilding !== null) {
      this.physicsObject.setTranslation(
        server.entities[playerParty.primaryBuilding].getPosition(server),
      );
    } else {
      const randomMapPosition = Util.randomMapPosition(
        server.serverProperties.mapSize,
      );
      this.physicsObject.setTranslation({
        x: randomMapPosition.x / server.world.PixelToWorld,
        y: randomMapPosition.y / server.world.PixelToWorld,
      });
    }
    ...
    for (let tool of this.tools)
      tools.push({ toolName: tool.toolName, toolTier: 0 });
  ...
  update(server) {
    ...
    if (this.health <= 0 && this.dead !== true && this.developerMode === false)
      this.die(server);
    ...
    if (this.inputs.aimingYaw !== this.aimingYaw)
      this.aimingYaw = this.inputs.aimingYaw;

    if (Object.values(this.inputs.movement).includes(true)) {
      this.physicsObject.applyImpulse(
        {
          x:
            (Math.sin((this.yaw * Math.PI) / 180) * this.movementSpeed) /
            (1000 / server.serverProperties.tickRate),
          y:
            (-Math.cos((this.yaw * Math.PI) / 180) * this.movementSpeed) /
            (1000 / server.serverProperties.tickRate),
        },
        true,
      );
    ...
    if (
      ...
      server.tick - this.firingTick >
        toolInfo.msBetweenFires / server.serverProperties.tickRate
    ) {
        ...
        case "Sword":
          {
            ...
            const shape = new RAPIER.Ball(
              (this.radius + toolInfo.range) / server.world.PixelToWorld,
            );
                ...
                if (entity !== undefined) {
                  ...
                  if (
                    ...
                    Util.isFacing(
                      this,
                      entity,
                      toolInfo.maxYawDeviation,
                      server,
                    ) === true
                  )
                    this.hit(server, entity);
                }
                return true;
              },
              undefined,
        ...
        case "Crossbow":
          {
            server.createEntity("ArrowProjectile", {
              ...
              position: {
                x:
                  this.getPosition(server).x * server.world.PixelToWorld +
                  Math.sin((this.aimingYaw * Math.PI) / 180) * 64,
                y:
                  this.getPosition(server).y * server.world.PixelToWorld -
                  Math.cos((this.aimingYaw * Math.PI) / 180) * 64,
              },
        ...
        case "Dynamite":
          {
            server.createEntity("DynamiteProjectile", {
              ...
              targetPosition: {
                x: Math.round(
                  this.inputs.mousePosition.x / server.world.PixelToWorld,
                ),
                y: Math.round(
                  this.inputs.mousePosition.y / server.world.PixelToWorld,
                ),
              },
              tickMade: server.tick,
    ...
    const ticksBeforeRegen =
      playerData.msBeforeRegen / server.serverProperties.tickRate;

    if (
      this.health < this.maxHealth &&
      (server.tick >= this.lastDamagedTick + ticksBeforeRegen ||
      ...
      if (this.developerMode === true) {
        this.health = Math.min(
          this.maxHealth,
          this.health +
            (playerData.healthRegenPerSecond / 250) *
        ...
        this.health = Math.min(
          this.maxHealth,
          this.health +
            (playerData.healthRegenPerSecond / 1000) *
              server.serverProperties.tickRate,
        );
      }
    }
    ...
    if (this.zombieShieldMaxHealth > 0) {
      const shieldInfo = ToolInfo.getTool(
        "ZombieShield",
        this.tools.find((e) => e.toolName == "ZombieShield").toolTier,
      );

      if (this.zombieShieldHealth < this.zombieShieldMaxHealth) {
        const ticksBeforeRegen =
          shieldInfo.msBeforeRegen / server.serverProperties.tickRate;

        if (
          server.tick >= this.lastShieldHitTick + ticksBeforeRegen ||
          this.developerMode === true
        ) {
          this.zombieShieldHealth = Math.min(
            this.zombieShieldMaxHealth,
            this.zombieShieldHealth +
              (shieldInfo.healthRegenPerSecond / 1000) *
                server.serverProperties.tickRate,
          );
        }
      }
    ...
    for (let timer in this.timers) {
      ...
      if (
        server.tick >
        this.timers[timer].lastTimerActive +
          this.timers[timer].delay / server.serverProperties.tickRate
      )
        this.timers[timer].active = false;
    }
  }
  ...
  hit(server, hitEntity) {
    ...
    switch (hitEntity.entityClass) {
      ...
      case "Building":
        ...
        if (this.weaponName == "Sword")
          damageToDeal = toolInfo.damageToBuildings * hitEntity.maxHealth;
    ...
    if (server.gameMode == "scarcity" && hitEntity.entityClass !== "Zombie")
      damageToDeal = 0;
  ...
  hitResponse(server, entity) {
    server.entities[server.parties[this.partyId]?.primaryBuilding]?.hitResponse(
      server,
      entity,
    );
  }
