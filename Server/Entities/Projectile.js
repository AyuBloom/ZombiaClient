...
class Projectile extends Entity {
  ...
  addToWorld(server) {
    ...
    server.waitTicks(
      this.lifetime / server.serverProperties.tickRate,
      this.die.bind(this, server),
    );
  }

  onCollision(entity, server) {
    if (
      entity.partyId == this.partyId ||
      this.dead == true ||
    ...
    switch (entity.entityClass) {
      ...
      case "Player":
        if (server.entities[this.parentUid].entityClass === "Player") {
          if (server.entities[this.parentUid].invulnerable == true)
            server.entities[this.parentUid].invulnerable = false;
        }

        if (
          server.entities[this.parentUid].entityClass === "Building" &&
          partyPrimaryBuilding !== undefined
        ) {
          if (partyPrimaryBuilding.checkEntityIsEnemy(server, entity) == false)
            return;
        }
      ...
      case "Zombie":
        ...
        if (
          server.gameMode == "scarcity" &&
          server.entities[this.parentUid].entityClass === "Player"
        ) {
  ...
  update(server) {
    ...
    if (
      ...
      this.model == "DynamiteProjectile" &&
      server.entities[this.parentUid] !== undefined
    ) {
      const distanceToPlayer = Util.measureDistance(
        server,
        this.getPosition(server),
        server.entities[this.parentUid].getPosition(server),
      );
      const targetPositionToPlayer = Util.measureDistance(
        server,
        this.targetPosition,
        server.entities[this.parentUid].getPosition(server),
      );
      ...
      if (distanceToPlayer > targetPositionToPlayer + 2)
        return this.die(server);
    }

    if (this.homing == true && this.target !== undefined) {
      if (
        server.entities[this.target] == undefined ||
    ...
    if (this.model === "RocketProjectile") {
      const timeSinceCreatedMs =
        (server.tick - this.tickMade) * server.serverProperties.tickRate;
      ...
      this.physicsObject.setLinvel({
        x:
          ...
          ((speed * server.serverProperties.tickRate) / 1000) *
  ...
  updateHoming(server) {
    ...
    if (target.model == "Player") {
      if (partyPrimaryBuilding.checkEntityIsEnemy(server, target) == false)
        return;
    }

    this.yaw = Math.floor(
      Util.angleTo(this.getPosition(server), target?.getPosition(server)),
    );
  }
}
