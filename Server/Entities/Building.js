...
class Building extends Entity {
  ...
  addToWorld(server) {
    const buildingInfo = BuildingInfo.getBuilding(
      server,
      this.model,
      this.tier,
    );
    ...
    const body = RAPIER.RigidBodyDesc.fixed()
      ...
      .setTranslation(
        this.position.x / server.world.PixelToWorld,
        this.position.y / server.world.PixelToWorld,
      );

    this.physicsObject = server.world.createRigidBody(body);

    const collider = RAPIER.ColliderDesc.cuboid(
      this.width / server.world.PixelToWorld / 2,
      this.height / server.world.PixelToWorld / 2,
    )
    ...
    this.ticksBeforeRegen =
      buildingInfo.msBeforeRegen / server.serverProperties.tickRate;
  ...
  updateRangeCells(server) {
    ...
    const primaryBuilding =
      server.entities[server.parties[this.partyId].primaryBuilding];
    ...
    if (buildingInfo.towerRadius !== undefined) {
      for (
        let x = thisPosition.x - buildingInfo.towerRadius / 48;
        x < thisPosition.x + buildingInfo.towerRadius / 48;
        x++
      ) {
        for (
          let y = thisPosition.y - buildingInfo.towerRadius / 48;
          y < thisPosition.y + buildingInfo.towerRadius / 48;
          y++
        ) {
          let x2;
          ...
          this.rangeCellsSorted[dist].push(
            primaryBuilding.enemyHashGrid[x2][y2],
          );
        }
      }
      ...
      for (let dist in this.rangeCellsSorted) {
        for (let cell of this.rangeCellsSorted[dist])
          this.sortedCellDistances.push(cell);
        this.sortedCellDistances.push(null);
      }
    } else if (buildingInfo.range !== undefined) {
      for (
        let x = thisPosition.x - buildingInfo.range / 48;
        x < thisPosition.x + buildingInfo.range / 48;
        x++
      ) {
        for (
          let y = thisPosition.y - buildingInfo.range / 48;
          y < thisPosition.y + buildingInfo.range / 48;
          y++
        ) {
          let x2;
          if (x < thisPosition.x) x2 = Math.floor(x);
  ...
  update(server) {
    ...
    if (primaryBuilding.lastUpdateTickRan !== server.tick)
      primaryBuilding.update(server);
    ...
    if (
      ...
      server.tick >= this.healthRegenStartTick
    ) {
      this.health =
        Math.round(
          (this.health +
            (buildingInfo.healthRegenPerSecond / 1000) *
    ...
    let msBetweenFires =
      this.speedBuffed == false
        ? buildingInfo.msBetweenFires
        : buildingInfo.msBetweenFires / 2;

    // Prevent buildings from attacking right after being created
    if (
      server.tick - this.tickMade >
        msBetweenFires / server.serverProperties.tickRate &&
  ...
  fireUnranged(server, buildingInfo) {
    ...
    for (const cell of this.rangeCells) {
      for (const uid of cell) {
        ...
        if (
          enemy !== undefined &&
          Util.isFacing(this, enemy, buildingInfo.maxYawDeviation)
        ) {
          enemyUids.push(uid);
        }
      }
  ...
  fireRanged(server, buildingInfo) {
    ...
    for (const cell of this.sortedCellDistances) {
      ...
      for (const uid of cell) {
        ...
        let distanceToEnemy = Util.measureDistance(
          server,
          thisPosition,
          enemyPosition,
        );
    ...
    this.handleTargetAttack(
      server,
      server.entities[closestEnemyUid],
      buildingInfo,
    );
  }

  handleTargetAttack(server, closestEnemy, buildingInfo) {
    this.aimingYaw = Math.floor(
      Util.angleTo(this.getPosition(server), closestEnemy.getPosition(server)),
    );
