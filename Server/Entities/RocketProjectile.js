...
class RocketProjectile extends Projectile {
  ...
  die(server) {
    ...
    const shape = new RAPIER.Ball(
      this.splashRadius / server.world.PixelToWorld,
    );
    ...
    server.world.intersectionsWithShape(
      ...
      (collider) => {
        ...
        if (partyPrimaryBuilding.checkEntityIsEnemy(server, entity) == true)
  ...
  onCollision(entity, server) {
    ...
    if (partyPrimaryBuilding.checkEntityIsEnemy(server, entity))
      this.die(server);
  }
}
