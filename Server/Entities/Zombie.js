...
class Zombie extends Entity {
  ...
  die(server, factoryDied = false) {
    if (factoryDied == false)
      server.entities[this.target]?.zombieDied(server, this.uid);
    super.die(server);
  }
  ...
  updatePath(server, overrideYaw = false) {
    ...
    if (this.movementMode == "Primary") {
      ...
      if (overrideYaw) {
        this.yaw = Math.floor(
          Util.angleTo(this.getPosition(server), targetPosition),
        );
      }
      return;
    }
    ...
    if (
      ...
      server.tick - this.pathfindFrequency / server.serverProperties.tickRate >
        this.lastPathUpdated ||
      // if the zombie is close to the current target
      Util.measureDistance(server, thisPosition, {
        x: this.targetPath[0],
        y: this.targetPath[1],
      }) < pathUpdateDistance
    ) {
      ...
      let tentativePath = targetParty.findPath(
      ...
      if (overrideYaw) {
        this.yaw = Math.floor(
          Util.angleTo(this.getPosition(server), {
            x: tentativePath[0],
            y: tentativePath[1],
          }),
        );
  ...
  update(server) {
    ...
    if (server.cycleData.isDay == true) {
      ...
      this.health =
        Math.round(
          (this.health -
            (server.serverProperties.tickRate / server.cycleData.dayLength) *
              2 *
              this.maxHealth) *
            100,
        ) / 100;
    ...
    const scoreDecayPerSecond =
      (scoreDecay / 1000) * server.serverProperties.tickRate;
    ...
    if (
      this.movementMode == "Primary" &&
      server.tick - this.tickMade > 3000 / server.serverProperties.tickRate
    ) {
      /*
      ...
      if (
        server.tick - this.lastPositionUpdatedTick >
        5000 / server.serverProperties.tickRate
      ) {
        const dist = Util.measureDistance(
          server,
          currentPosition,
          this.lastPosition,
        );
    ...
    this.targetYaw = Util.angleTo(this.getPosition(server), {
      x: this.targetPath[0],
      y: this.targetPath[1],
    });
    ...
    this.physicsObject.applyImpulse(
      {
        x:
          (Math.sin((this.yaw * Math.PI) / 180) * this.acceleration) /
          (1000 / server.serverProperties.tickRate),
        y:
          (-Math.cos((this.yaw * Math.PI) / 180) * this.acceleration) /
          (1000 / server.serverProperties.tickRate),
      },
      true,
    );
    ...
    if (
      ...
      this.msBetweenFires / server.serverProperties.tickRate
    ) {
  ...
  damagePlayer(server, entity) {
    ...
    if (
      ...
      server.entities[this.target]?.partyId !== entity.partyId
    ) {
    ...
    if (entity.zombieShieldMaxHealth > 0) {
      ...
      entity.zombieShieldHealth = Math.max(
        0,
        entity.zombieShieldHealth - damageToBeDealt,
      );
  ...
  damageBuilding(server, entity) {
    ...
    if (this.finalZombie == true) {
      damageToBeDealt = Math.min(
        Math.round(damageToBeDealt * (server.entities[this.target].wave - 42)),
        1000,
      );
    }
  ...
  hitResponse(server, entity) {
    ...
    const scoreMultiplier =
      EntityInfo.entityInfo.Zombie.scoreFactors.towerHits[tierDifference];
