...
class HarvesterDrone extends Entity {
  ...
  die(server) {
    ...
    parentHarvester.droneCount = Object.keys(
      parentHarvester.childDrones,
    ).length;

    const buildingInfo = BuildingInfo.getBuilding(
      server,
      parentHarvester.model,
      parentHarvester.tier,
    );
    ...
    for (const resource in this.resourcesCollected) {
      server.createEntity("ResourcePickup", {
        ...
        ticksUntilExpiry:
          buildingInfo.resourcePickupDurationMs /
          server.serverProperties.tickRate,
        pickupRange:
          buildingInfo.resourcePickupPlayerPickupRange /
          server.world.PixelToWorld,
        position: {
          x: this.getPosition(server).x * server.world.PixelToWorld,
          y: this.getPosition(server).y * server.world.PixelToWorld,
        },
  ...
  addToWorld(server) {
    ...
    this.generateRandomSpaceAroundTarget(
      server,
      parentHarvester.getPosition(server),
      5,
    );

    this.spawningTimeoutActive = true;
    server.waitTicks(1000 / server.serverProperties.tickRate, () => {
      this.spawningTimeoutActive = false;
    });
  }
  ...
  update(server) {
    ...
    const ticksBeforeRegen =
      this.msBeforeHealthRegen / server.serverProperties.tickRate;
    ...
    if (
      ...
      this.health = Math.min(
        this.maxHealth,
        this.health +
          (this.healthRegenPercentPerSecond / 1000) *
            server.serverProperties.tickRate *
            this.maxHealth,
      );
    }
  ...
  updateMovement(server) {
    ...
    if (this.targetResourceUid == 0) {
      ...
      this.yaw = Math.floor(
        Util.angleTo(this.getPosition(server), this.randomTarget),
      );
    ...
    const distToTargetResource = Math.sqrt(
      Util.measureDistance(
        server,
        this.getPosition(server),
        targetResource.getPosition(server),
      ),
    );
      ...
      case this.harvestStages["enroute"]:
        {
          ...
          if (
            distToTargetResource <=
            buildingInfo.droneCollectRange / server.world.PixelToWorld
          ) {
      ...
      case this.harvestStages["collecting"]:
        {
          ...
          if (
            currentTick - this.lastChargeTick >
            buildingInfo.droneChargeFrequency / server.serverProperties.tickRate
          ) {
          ...
          if (this.chargingAtResource == true) {
            if (distToTargetResource <= 1) {
              ...
              this.resourcesCollected[resourceType] +=
                buildingInfo.droneCollectRate;

              if (
                this.resourcesCollected[resourceType] >=
                buildingInfo.droneMaxResources
              ) {
            ...
            const distToRandomSpace = Math.sqrt(
              Util.measureDistance(
                server,
                this.getPosition(server),
                this.randomTarget,
  ...
  applyImpulseToYaw(server, speed) {
    this.physicsObject.applyImpulse(
      {
        x:
          (Math.sin((this.yaw * Math.PI) / 180) * speed) /
          (1000 / server.serverProperties.tickRate),
        y:
          (-Math.cos((this.yaw * Math.PI) / 180) * speed) /
          (1000 / server.serverProperties.tickRate),
      },
      true,
    );
