...
class Factory extends Building {
  ...
  addToWorld(server) {
    ...
    this.enemyHashGrid = {};
    for (
      let x = thisPosition.x - this.maxRange;
      x < thisPosition.x + this.maxRange;
      x += gridSize
    ) {
      this.enemyHashGrid[x] = {};
      for (
        let y = thisPosition.y - this.maxRange;
        y < thisPosition.y + this.maxRange;
        y += gridSize
      ) {
  ...
  hitResponse(server, entity) {
    ...
    if (entity.parentUid !== undefined)
      entity = server.entities[entity.parentUid];
    ...
    if (
      this.aggressionData.playerHits[entity.uid] >=
      this.aggressionData.hitThreshold
    )
      this.aggressionData.playerUids.add(entity.uid);
    ...
    if (
      ...
      this.aggressionData.partyIds.add(originalPartyId);
    ...
    if (
      ...
      this.aggressionData.playerIps.add(playerIpAddress);

    server.waitTicks(
      this.aggressionData.timeForHitToDecay / server.serverProperties.tickRate,
      () => {
        this.aggressionData.playerHits[entity.uid]--;
  ...
  startWave(server) {
    ...
    const checkColourInRange = (wave, colourIndex = 0) => {
      ...
      if (wave - zombieColour.waveDuration <= 0)
        return {
          waveInColour: wave,
          ...WaveData[WaveDataKeys[colourIndex]],
        };
      else
        return checkColourInRange(
          wave - zombieColour.waveDuration,
          colourIndex + 1,
        );
    };
    ...
    const tierPercentage =
      (this.waveData.zombieTierCount / this.waveData.waveDuration) *
      this.waveData.waveInColour;
    ...
    if (this.waveData.finalZombie == true) {
      ...
      this.waveData.tiersToSpawn = [
        Math.max(
          1,
          Math.min(this.waveData.zombieTierCount, Math.floor(tierPercentage)),
        ),
        Math.max(
          1,
          Math.min(this.waveData.zombieTierCount, Math.ceil(tierPercentage)),
        ),
      ];
    }

    const ticksToSpawn =
      server.cycleData.nightLength / server.serverProperties.tickRate / 2;
    const ticksBetweenZombieSpawns =
      ticksToSpawn /
      this.waveData.zombieSpawnEventCount[this.waveData.waveInColour - 1];

    this.waveData.zombieSpawnTicks = [];

    for (
      let i = server.cycleData.cycleStartTick;
      i < server.cycleData.cycleStartTick + ticksToSpawn;
      i += ticksBetweenZombieSpawns
    ) {
      this.waveData.zombieSpawnTicks.push(Math.round(i + 1));
    }

    for (let i = 0; i < 0; i++) {
      const position = this.calculateZombieSpawnPosition(
        server,
        thisParty.radialDistribution,
      );
      const tier =
        this.waveData.tiersToSpawn[
          Math.floor(Math.random() * this.waveData.tiersToSpawn.length)
        ];

      this.createZombie(
        server,
        this.waveData.colour,
        position,
        tier,
        this.waveData.finalZombie,
      );
    }
  }
  ...
  calculateZombieSpawnPosition(server, buildingDistribution) {
    ...
    const spawnX =
      (this.getPosition(server).x + Math.cos(sectorAngle) * minDistance) *
      server.world.PixelToWorld;
    const spawnY =
      (this.getPosition(server).y + Math.sin(sectorAngle) * minDistance) *
      server.world.PixelToWorld;
    ...
    const x = Math.max(
      0,
      Math.min(
        server.serverProperties.mapSize.width,
        spawnX + Math.floor(Math.random() * 192) * Math.cos(randomAngle),
      ),
    );
    const y = Math.max(
      0,
      Math.min(
        server.serverProperties.mapSize.height,
        spawnY + Math.floor(Math.random() * 192) * Math.sin(randomAngle),
      ),
    );
  ...
  async die(server) {
    ...
    for (let uid in thisParty.allBuildings.buildings) {
      if (parseInt(uid) !== this.uid)
        thisParty.allBuildings.buildings[parseInt(uid)].die(server);
    }
  ...
  async sendLeaderboardToDatabase(server) {
    return new Promise(async (res, rej) => {
      ...
      const playersArray = Object.values(thisParty.members)
        .filter((member) => {
      ...
      const leaderboardEntryModel = mongoose.model(
        ...
        `leaderboard_v${server.gameVersion}`,
      );
      ...
      server.debugLog(
        `[LEADERBOARD-UPDATE] ${leaderboardEntry._id} has been saved to the leaderboard`,
      );
  ...
  zombieDied(server, uid) {
    if (this.dead !== true) {
      ...
      for (let memberUid in server.parties[this.partyId].members) {
        server.entities[memberUid].score += Math.round(
          scoreDistributed / server.parties[this.partyId].memberCount,
        );
      }

      if (zombieEntity.killedByUid !== 0) {
        const killedByEntity = server.entities[zombieEntity.killedByUid];
        if (killedByEntity !== undefined && killedByEntity.model == "Player")
          killedByEntity.score += Math.round(scoreDistributed);
      }
    }
  ...
  update(server) {
    ...
    if (this.warmingUp == true) {
      ...
      const healthStepToMax =
        this.maxHealth / (timeToWarmUp / server.serverProperties.tickRate);
      this.health = Math.min(this.maxHealth, this.health + healthStepToMax);

      if (
        server.tick - this.tickMade >=
        timeToWarmUp / server.serverProperties.tickRate
      )
    ...
    server.world.intersectionsWithShape(
      this.getPosition(server),
      0,
      shape,
      (collider) => {
        ...
        if (entity !== undefined && entity.dead !== true) {
          if (this.checkEntityIsEnemy(server, entity) == true) {
            ...
            if (
              ...
              this.enemyHashGrid[entityTopLeft.x][entityTopLeft.y] ||= [];
              this.enemyHashGrid[entityTopLeft.x][entityTopLeft.y].push(
                entity.uid,
              );
              this.changedGridCells.add(
                this.enemyHashGrid[entityTopLeft.x][entityTopLeft.y],
              );
            }
            if (
              ...
              this.enemyHashGrid[entityTopRight.x][entityTopRight.y] ||= [];
              this.enemyHashGrid[entityTopRight.x][entityTopRight.y].push(
                entity.uid,
              );
              this.changedGridCells.add(
                this.enemyHashGrid[entityTopRight.x][entityTopRight.y],
            ...
            if (
              ...
              this.enemyHashGrid[entityBottomLeft.x][entityBottomLeft.y] ||= [];
              this.enemyHashGrid[entityBottomLeft.x][entityBottomLeft.y].push(
                entity.uid,
              );
              this.changedGridCells.add(
                this.enemyHashGrid[entityBottomLeft.x][entityBottomLeft.y],
            ...
            if (
              ...
              this.enemyHashGrid[entityBottomRight.x][entityBottomRight.y] ||=
                [];
              this.enemyHashGrid[entityBottomRight.x][entityBottomRight.y].push(
                entity.uid,
              );
              this.changedGridCells.add(
                this.enemyHashGrid[entityBottomRight.x][entityBottomRight.y],
  ...
  checkEntityIsEnemy(server, entity) {
    ...
    if (entity.entityClass == "Player") {
      ...
      if (
        this.aggressionData.partyIds.has(entity.partyId) ||
        this.aggressionData.playerUids.has(entity.uid) ||
        this.aggressionData.playerIps.has(
          server.connectedSockets[entity.uid]?.ipAddress,
        )
      )
        return true;
    }
  ...
  spawnZombies(server) {
    ...
    if (
      server.tick - server.cycleData.cycleStartTick >
      server.cycleData.nightLength / server.serverProperties.tickRate / 2
    )
      return;
    ...
    if (this.waveData.zombieSpawnTicks.includes(server.tick)) {
      ...
      let zombiesPerSpawnTick =
        this.waveData.zombiesSpawnedInWave[this.waveData.waveInColour - 1] /
        this.waveData.zombieSpawnEventCount[this.waveData.waveInColour - 1];
      ...
      let lastPosition = this.calculateZombieSpawnPosition(
        server,
        thisParty.radialDistribution,
      );
      for (let i = 0; i < thisParty.memberCount * zombiesPerSpawnTick; i++) {
        ...
        // for (let i = 0; i < 0; i++) {
        let position = this.calculateZombieSpawnPosition(
          server,
          thisParty.radialDistribution,
        );
  ...
  createZombie(server, colour, position, tier, finalZombie) {
    ...
    let pathfindFrequency =
      Math.floor(
        (Math.random() * (3000 - 2000 + 1)) / server.serverProperties.tickRate,
      ) *
        server.serverProperties.tickRate +
      2000;
